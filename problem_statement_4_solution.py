def solution(arr):
    half = len(arr) // 2
    most_occurring = max(arr, key=lambda x: arr.count(x))
    if most_occurring > half:
        return 1
    else:
        return -1
"""
...
For example, given array A consisting of ten elements such that:
  ...
  */
  ...
  2. each element of array A is an integer within the range [0..2,147,483,647]
  3. array A is sorted in non-decreasing order.
