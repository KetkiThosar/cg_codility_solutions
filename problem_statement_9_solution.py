# the matrix should be square to find saddle point.
def solution(matrix):
    saddle_counter = 0
    for (r, c) in zip(matrix, list(zip(*matrix))):
        if min(r) == max(c) or min(c) == max(r):
            saddle_counter += 1
    return saddle_counter


# the example matrix contains 2 saddle points.

matrix = [[10, 12, 7, 3, 12], [3, 10, 6, 2, 8], [18, 24, 17, 6, 10],
          [15, 21, 10, 8, 12], [1, 18, 22, 4, 15]]
